...
'''
An overlap alignment between two strings is a maximally scoring prefix of string w alignment with a suffix of string
v. This alignment is performed by forming a scoring matrix of the longest path to every node in an alignment graph
and recording the path taken to each node in a backtrack matrix. The construction of the aligned affixes is initiated
by starting from the maximally scoring node along the bottom row, which represents the last character of v's suffix
and backtracking until the first column is reached, which represents the first character of the prefix of string w.
'''
...
def overlap_alignment(v, w, i, j, match, sigma, mu): # v = vertical, w = horizontal
...
overlap_alignment(v, w, len(v), len(w), match, sigma, mu)


# Overlap Alignment Score: 16
# WYHEY
# WWHEY